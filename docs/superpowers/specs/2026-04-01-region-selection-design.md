...
### 4. CSP 绕过：MAIN → ISOLATED → Background 消息中继

MAIN world 脚本受页面 CSP 限制，无法 fetch 到 `127.0.0.1`（Wikipedia 等严格 CSP 站点会拦截）。解决方案：

```
region-selector.js (MAIN world)
  → window.postMessage({ type: "shibei:save-region", payload })
    → relay.js (ISOLATED world, 监听 window message)
      → chrome.runtime.sendMessage({ type: "save-region", data })
        → background.js (Service Worker, 无 CSP 限制)
          → fetch POST /api/save
          → 返回结果
        ← sendResponse
      ← window.postMessage({ type: "shibei:save-region-result" })
    ← region-selector.js 监听结果，显示 toast
```

同理，`chrome.storage.session` 也不可用于 MAIN world，保存参数通过 `window.__shibeiSaveParams` 全局变量传递。
...
| 插件选区脚本 | `content/region-selector.js` | **新增**：选区交互 + 裁剪 |
| 插件中继脚本 | `content/relay.js` | **新增**：ISOLATED world 消息中继 |
| 插件 background | `background/background.js` | 修改：新增 save-region 处理 |
