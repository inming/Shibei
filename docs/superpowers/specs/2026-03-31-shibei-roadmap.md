...
MVP 已完成（Phase 1-8）。**v1.1 全部完成。v1.1.1 全部完成。v1.2 全部完成。v1.2.1 全部完成。v1.3 全部完成（E2EE → v1.3.1）。v1.3.1 全部完成。v1.3.2 全部完成。v1.3.3 全部完成。v1.4 第一期完成（元数据搜索，全文搜索移至 v2.0）。下一步：v1.5 深色模式 → v1.6 Markdown 标注 → v1.7 MCP Server → v1.8 i18n。**
...
- [ ] **高亮颜色适配** — 深色模式下高亮颜色调整
- [ ] **快照内容适配** — iframe 内网页快照的深色模式处理策略（CSS filter / 保持原样 / 用户可选）
...
### 编辑器
- [ ] **Markdown 编辑** — 评论和资料级笔记输入框支持 Markdown 语法（标题、列表、粗斜体、代码块、链接等）
- [ ] **实时预览** — 编辑时实时渲染预览，或切换编辑/预览模式
- [ ] **轻量编辑器选型** — 评估方案：纯 textarea + 渲染预览 / 轻量所见即所得（如 Milkdown、tiptap）

### 渲染
- [ ] **评论 Markdown 渲染** — AnnotationPanel 中评论内容按 Markdown 渲染展示
- [ ] **笔记 Markdown 渲染** — 资料级笔记、PreviewPanel 笔记区域 Markdown 渲染
- [ ] **代码高亮** — 代码块语法高亮（可选，视需求）

### 兼容性
- [ ] **数据兼容** — 已有纯文本评论无需迁移，渲染时自动兼容（纯文本即合法 Markdown）
- [ ] **搜索索引** — FTS5 索引内容保持纯文本（strip Markdown 语法后索引）
