...
(function () {
  ...
  `;
     ...
     */
    function resolveByPosition(anchor, cachedTextNodes) {
     ...
     */
    function resolveByQuote(anchor, cachedTextNodes) {
        const textNodes = cachedTextNodes ?? getTextNodes(document.body);
     ...
     */
    function resolveAnchor(anchor, cachedTextNodes) {
    ...
    document.addEventListener("mouseup", (e) => {
        ...
        if (!selection || selection.isCollapsed || !selection.toString().trim()) {
            window.parent.postMessage({ type: "shibei:selection-cleared", source: "shibei" }, "*");
        }
    });
    ...
    function findHighlightElement(target) {
        let node = target;
        while (node) {
            if (node.nodeType === Node.ELEMENT_NODE && node.tagName === "SHIBEI-HL" && node.hasAttribute("data-hl-id")) {
                return node;
            }
    ...
    window.addEventListener("message", (event) => {
        ...
        if (!msg || !msg.type || msg.source !== "shibei")
            return;
        switch (msg.type) {
            ...
            case "shibei:update-highlight-color":
                if (msg.id && msg.color) {
                    document.querySelectorAll(`shibei-hl[data-hl-id="${msg.id}"]`).forEach((el) => {
                        el.style.setProperty("--shibei-hl-color", msg.color);
                        el.style.setProperty("--shibei-hl-text", getContrastText(msg.color));
                    });
                }
                break;
    ...
    function signalReady() {
        const readyMsg = { type: "shibei:annotator-ready", source: "shibei" };
