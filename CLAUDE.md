...
- **Deep Link**：`shibei://open/resource/{id}?highlight={hlId}` 与 `shibei://open/folder/{id}` 三种入口：`onOpenUrl`（应用已开）、`deep-link-received`（单实例转发）、`getCurrent()`（冷启动）。锁屏时暂存到 `pendingDeepLinkRef`，解锁后自动打开目标资料或文件夹
