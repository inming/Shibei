...
crates/
  shibei-db/            # 数据层：SQLite schema + CRUD + FTS5 + HLC + sync_log + SyncContext + questions（+ 10 SQL migrations）
  shibei-events/        # 11 个领域事件名常量（desktop + mobile 共享）
...
src-tauri/          # Rust 后端（Tauri core — Phase 2 A2 后仅保留 Tauri 集成层）
  src/
    commands/       # Tauri command handlers（54 个命令，含 cmd_search_resources/cmd_export_backup/cmd_import_backup/cmd_get_ai_tool_paths/cmd_generate_pairing_payload/cmd_import_audio/cmd_get_transcript；question 系列拆到 commands/questions.rs 子模块）
...
- **MCP Server**：独立 Node.js 进程，stdio transport，通过 HTTP 调用主应用 axum server（需主应用运行）。Token 文件 `{data_dir}/mcp-token` 鉴权。17 个工具：search_resources, get_resource (audio 资源额外返回 `abs_path` 供 agent 读文件转写), get_annotations, get_resource_content, set_transcript (写 transcript.json + plain_text，音频转写委托外部 agent), list_folders, list_tags, update_resource, manage_tags, manage_notes, manage_folders, sync_now, list_questions, get_question (with include_linked), manage_questions, link_to_question, unlink_from_question
- **问题系统（Questions）**：独立的"研究焦点"实体，跨 `resource` / `highlight` / `comment` 多态链接，作为 AI 阶段总结的主入口（`get_question(id, include_linked=true)`）。两张表 `questions` + `question_links`，HLC LWW + sync_log 走既有同步通道，partial UNIQUE 防一对一关联重复。详见 `docs/superpowers/specs/2026-05-27-questions-system-design.md`
