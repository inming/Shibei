...
src-tauri/          # Rust 后端（Tauri core — Phase 2 A2 后仅保留 Tauri 集成层）
  src/
    commands/       # Tauri command handlers（52 个命令，含 cmd_search_resources/cmd_export_backup/cmd_import_backup/cmd_get_ai_tool_paths/cmd_generate_pairing_payload；question 系列拆到 commands/questions.rs 子模块）
...
src/                # React 前端
  components/       # UI 组件（Layout, TabBar, ReaderView, PDFReader, AnnotationPanel, MarkdownContent, SettingsView, Settings/AIPage/..., Sidebar/..., Sidebar/QuestionSection|QuestionItem|QuestionEditDialog|LinkToQuestionSubMenu|LinkToQuestionPopover, QuestionDetail/QuestionDetailView|QuestionLinkItem）
...
- **MCP Server**：独立 Node.js 进程，stdio transport，通过 HTTP 调用主应用 axum server（需主应用运行）。Token 文件 `{data_dir}/mcp-token` 鉴权。16 个工具：search_resources, get_resource, get_annotations, get_resource_content, list_folders, list_tags, update_resource, manage_tags, manage_notes, manage_folders, sync_now, list_questions, get_question (with include_linked), manage_questions, link_to_question, unlink_from_question
