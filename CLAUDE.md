...
src/                # React 前端
  ...
  hooks/            # 自定义 hooks（useFolders, useResources, useTags, useAnnotations, useSync, useFlipPosition — 菜单/popover 边界防溢出；useQuestions/useQuestionLinks/useQuestionsForTarget — questions 系统）
  stores/           # 状态管理
...
- **鸿蒙 Questions（Phase H，2026-05-28）**：14 个 NAPI 命令（list/get/create/update/archive/unarchive/delete/list_links/list_for_target/link/update_link_reason/unlink/search + 2 个 lookup `get_highlight`/`get_comment` 给 Detail 反向解析），全部通过 `shibei-napi-codegen` 生成；ArkTS 入口 `services/QuestionService.ets` 是单例 façade，持三层缓存（active/archived list snapshot、per-question links、per-target reverse-lookup）+ 各自订阅器；构造时自挂 `ShibeiService.onDataChanged`，远端同步完成自动 invalidate UI。UI：`pages/Questions.ets`（active/archived 折叠组 + + 按钮 + 长按菜单：编辑/复制链接/归档/删除）；`pages/QuestionDetail.ets`（状态徽章 + Markdown 描述 + 按 target_type 分组的链接列表 + 点击跳转 Reader + ⋯ 菜单）；`pages/QuestionEdit.ets`（创建/编辑表单）；`components/QuestionLinkSheet.ets` 是底部 sheet，**点击 toggle 后自动关闭**（对齐桌面 LinkToQuestionSubMenu），行用 `@Styles` + `.stateStyles({ pressed, normal })` 加按压反馈。`components/ResourceItem.ets` 卡片底部显示反查 chip 行（≤3 个 + "+N" 溢出）。**ArkUI @Builder 限制**：禁止在 @Builder 体内写 `const` / `if-return`——QuestionDetail 把 link 分组过滤搬到调用方（`linksByType(type): QuestionLink[]` 辅助方法），把 `resolved.get(linkId)` 字段访问改成 `resolvedTitle(linkId): string \| null` 等方法。Deep link `shibei://open/question/{id}` 在 `EntryAbility.onNewWant`（已运行）和 `Library.consumePendingDeepLink`（冷启动/锁屏后）双路径处理。**评论无长按菜单**——只 note 卡（`CommentCard` builder，实际仅渲染 highlight_id IS NULL 的 comment）有 `NoteContextMenu`
