...
- 问题（Questions）系统设计：`docs/superpowers/specs/2026-05-27-questions-system-design.md`
- 问题研究笔记设计：`docs/superpowers/specs/2026-06-02-question-research-notes-design.md`
- 音频资料支持设计：`docs/superpowers/specs/2026-05-31-audio-support-design.md`
- 许可证：AGPL-3.0
...
crates/
  shibei-db/            # 数据层：SQLite schema + CRUD + FTS5 + HLC + sync_log + SyncContext + questions（+ 12 SQL migrations）
  shibei-events/        # 12 个领域事件名常量（desktop + mobile 共享）
...
src-tauri/          # Rust 后端（Tauri core — Phase 2 A2 后仅保留 Tauri 集成层）
  src/
    commands/       # Tauri command handlers（58 个命令，含 cmd_search_resources/cmd_export_backup/cmd_import_backup/cmd_get_ai_tool_paths/cmd_generate_pairing_payload/cmd_import_audio/cmd_get_transcript；question 系列拆到 commands/questions.rs 子模块，含 cmd_*_question_note 研究笔记 CRUD）
...
src/                # React 前端
  components/       # UI 组件（Layout, TabBar, ReaderView, PDFReader, AudioReader, AnnotationPanel, MarkdownContent, SettingsView, Settings/AIPage/..., Sidebar/..., Sidebar/QuestionSection|QuestionItem|QuestionEditDialog|LinkToQuestionSubMenu|LinkToQuestionPopover, QuestionDetail/QuestionDetailView|QuestionLinkItem）
  hooks/            # 自定义 hooks（useFolders, useResources, useTags, useAnnotations, useSync, useFlipPosition — 菜单/popover 边界防溢出；useQuestions/useResolvedQuestionLinks/useQuestionsForTarget — questions 系统）
  stores/           # 状态管理
  types/            # TypeScript 类型定义（含 i18next.d.ts 类型增强）
  lib/              # 工具函数（commands.ts — Tauri invoke 封装 + translateError；importPdf.ts — 右键菜单共享的 PDF 导入逻辑；sessionState.ts — 会话持久化 localStorage 存储；deepLink.ts — shibei:// URL 解析/构造）
  styles/           # CSS 变量 + 全局样式
  locales/          # i18n 语言包（zh/en 各 12 个 JSON 命名空间）
...
mcp/                # MCP Server（Node.js，stdio transport）
  src/
    ...
    tools/          # MCP 工具实现（search, resource, annotations, folders, tags, sync, questions）
    types.ts        # 共享类型定义
```
...
- **搜索**：FTS5 trigram 虚拟表 `search_index`，CRUD 操作后自动维护索引（best-effort），前端 ResourceList 顶部搜索框（固定不随列表滚动）debounce 300ms 触发，标题/URL/标注匹配高亮。快照正文通过 `body_text` 列全文索引。`SearchResult` 返回 `match_fields`（匹配字段列表）和 `snippet`（正文上下文片段），前端显示匹配类型标签（正文匹配/标注匹配/评论匹配）和 snippet 高亮。**问题独立 FTS**：`question_index` 表（trigram 索引 title+description+notes_text，migration 012 起含研究笔记内容），搜索栏 ≥2 字符并行调 `cmd_search_questions`（返回 `QuestionSearchResult` = `Question` flatten + `matchFields` + `snippet`），匹配命中以 chip 行显示在列表上方；命中在标题之外（描述/笔记）时 chip 展开为小卡片显示匹配标签（`descriptionMatch`/`notesMatch`）+ 高亮 snippet
- **MCP Server**：独立 Node.js 进程，stdio transport，通过 HTTP 调用主应用 axum server（需主应用运行）。Token 文件 `{data_dir}/mcp-token` 鉴权。18 个工具：search_resources, get_resource (audio 资源额外返回 `abs_path` 供 agent 读文件转写), get_annotations, get_resource_content, set_transcript (写 transcript.json + plain_text，音频转写委托外部 agent), list_folders, list_tags, update_resource, manage_tags, manage_notes, manage_folders, sync_now, list_questions, get_question (with include_linked；输出含研究笔记), manage_questions, link_to_question, unlink_from_question, manage_question_notes (研究笔记 create/update/delete——AI 阶段总结的落地出口)
- **问题系统（Questions）**：独立的"研究焦点"实体，跨 `resource` / `highlight` / `comment` 多态链接，作为 AI 阶段总结的主入口（`get_question(id, include_linked=true)`）。三张表 `questions` + `question_links` + `question_notes`（研究笔记），HLC LWW + sync_log 走既有同步通道，partial UNIQUE 防一对一关联重复。详见 `docs/superpowers/specs/2026-05-27-questions-system-design.md` 与 `docs/superpowers/specs/2026-06-02-question-research-notes-design.md`
...
- **Questions 系统**：`questions` + `question_links` 表（migration 008），独立 FTS5 `question_index`（migration 009）。**status (active/archived) 与 deleted_at 正交**——归档不动 links，删除才级联软删。`question_links` 是多态：`target_type ∈ {resource, highlight, comment}`；partial UNIQUE `(question_id, target_type, target_id) WHERE deleted_at IS NULL` 防一对一关联重复，远端冲突由 `upsert_question_link` 按 HLC 决定胜者（loser 软删，incoming loser 以 `deleted_at != NULL` 插入保留历史）。**Note (资料级笔记) = comment with NULL highlight_id**，UI 上**资料/高亮/笔记**可关联问题，**评论（attached to highlight）不暴露关联 UI**（数据层仍支持 `target_type='comment'`，MCP/编程可建——理由：高亮粒度已足够，评论是高亮的上下文）。Folder 软删时 `cascade_folder_delete` 自动级联清掉指向其 resources/highlights/comments 的 question_links。事件发射对：`mutation → data:question-changed` 或 `data:question-link-changed`。前端组件树：Sidebar `QuestionSection` + Question Detail Tab（lazy-mount，session 持久化经 `questionTabs` 平行数组而非 tagged union 以避免老 session 文件迁移）+ `LinkToQuestionSubMenu`（hover 子菜单，资料/高亮右键）+ `LinkToQuestionPopover`（独立 popover，笔记右键）+ PreviewPanel 反查 chip（`useQuestionsForTarget` hook）。详见 `docs/superpowers/specs/2026-05-27-questions-system-design.md`
- **问题研究笔记（Question Research Notes，migration 011）**：`question_notes` 表（多条/问题，markdown `content`），**独立同步实体**（仿 `question_links`，id-based HLC LWW，无 UNIQUE），与 question 行解耦——note 编辑不与 title/status 行级 LWW 互相覆盖。`create/update/delete_question_note`（`questions.rs`）+ `delete_question` 级联软删其 notes（每条写 sync_log DELETE）。同步五处接入：`upsert_question_note` 路由 / `soft_delete_entity` 表映射 / `get_entity_hlc` 表映射 / `topo_order`（INSERT 跟在 question 后、DELETE 在 question 前）/ 全量快照 `FullSnapshot.question_notes` export+import / compaction + `purge_entity`。事件 `data:question-note-changed`（独立于 question-changed，避免侧栏无谓刷新）。排序 `created_at DESC`（最新在上，编辑不改位置）。MCP 可读写：`get_question` 输出含笔记 + `manage_question_notes`——AI 读问题+证据后把阶段总结写成一条笔记沉淀。桌面 UI：`QuestionDetailView` 内 `QuestionNotesSection`（描述与关联之间）+ `useQuestionNotes` hook（textarea+预览切换，inline 编辑）。**全文搜索（Phase 3，已实现）**：migration 012 重建 `question_index` 加 `notes_text` 列并**在迁移内 SQL 回填**（`INSERT…SELECT` + `group_concat` 笔记内容；因鸿蒙端无 question-FTS boot pass，迁移内回填是唯一平台无关、原子重建两端索引的机制，不重置 `config:question_fts_initialized`）；`rebuild_question_search_index` 聚合该问题所有 alive 笔记进 `notes_text`（FTS5 MATCH 自动覆盖该列，≥3 字符查询免改即命中笔记；2 字符 LIKE 兜底加 `OR qi.notes_text LIKE`）；笔记 CRUD（Phase 1 已埋的 hook）+ sync apply（`affected_question_ids` 对 `question_note` 取 `payload.question_id`）触发重建。搜索命中返回**问题**（打开详情看笔记），与既有问题搜索一致。详见 `docs/superpowers/specs/2026-06-02-question-research-notes-design.md`
- **i18n 国际化**：`react-i18next` + `i18next`，12 个命名空间（common/sidebar/reader/annotation/settings/sync/encryption/lock/search/data/ai/question）。语言包静态导入（`src/locales/zh/*.json` + `src/locales/en/*.json`），`src/i18n.ts` 初始化。**新增 UI 文案必须使用 `t('key')` 而非硬编码中文**，同时更新 zh 和 en 两个 JSON 文件。`useTranslation('namespace')` 在组件/hooks 中获取 `t` 函数。后端错误消息返回 i18n key（`error.xxx` 格式），前端 `translateError()` 翻译。Chrome 插件使用 `chrome.i18n` API（`_locales/` 目录），MAIN world 脚本通过注入参数接收翻译字符串。语言持久化 `localStorage: shibei-language`，检测顺序 `localStorage → navigator`。TypeScript 类型安全通过 `src/types/i18next.d.ts` 的 `CustomTypeOptions` 增强
- **数据变更事件**：9 个领域事件（`data:resource-changed`、`data:folder-changed`、`data:tag-changed`、`data:annotation-changed`、`data:question-changed`、`data:question-link-changed`、`data:question-note-changed`、`data:sync-completed`、`data:config-changed`）+ 2 个同步状态事件（`sync-started`、`sync-failed`）。后端事件常量定义在 `crates/shibei-events/src/lib.rs`（facade re-export 为 `crate::events`），前端常量和 payload 类型定义在 `src/lib/events.ts`。**新增 mutation command 必须在 DB 写入后 emit 对应领域事件**；新增 hook 需检查订阅矩阵（见 `docs/superpowers/specs/2026-04-03-unified-data-events-design.md`）。禁止使用 `onDataChanged` 回调链、`refreshKey` state、`folderTreeRefreshRef` 等旧机制
- **单实例**：`tauri-plugin-single-instance` 确保同时只有一个应用实例运行。第二个实例启动时通过 `deep-link-received` 事件将 URL 转发给已有窗口，并聚焦已有窗口
- **Deep Link**：`shibei://open/resource/{id}?highlight={hlId}`、`shibei://open/folder/{id}`、`shibei://open/question/{id}` 三种入口：`onOpenUrl`（应用已开）、`deep-link-received`（单实例转发）、`getCurrent()`（冷启动）。锁屏时暂存到 `pendingDeepLinkRef`，解锁后自动打开目标资料/文件夹/问题。所有 URL 通过 `src/lib/deepLink.ts` 的 `parseShibeiDeepLink` / `buildResourceDeepLink` / `buildFolderDeepLink` / `buildQuestionDeepLink` 统一处理，禁止手拼字符串
...
- **预览面板**：概览模式显示摘要（`description` > `plain_text` 前 200 字 > 暂无摘要）+ 高亮/评论内容列表。`cmd_get_resource_summary` 命令从 `plain_text` 提取前 N 字符。不显示打开按钮（双击打开即可）。**元数据编辑**：`ResourceMeta` 右上角铅笔按钮打开 `ResourceEditDialog` 编辑 title/description/url/`content_time`（资料时间，date 输入，可回补/清空）；编辑后由 `data:resource-changed` 事件驱动 PreviewPanel 自动刷新。`content_time` 有值时 `ResourceMeta` 额外显示「资料时间」行。**高亮可点击跳转**：PreviewPanel 每条高亮可点击（含键盘 Enter/Space），通过 Layout 传入的 `onOpenHighlight` 回调调用 `openResource(resource, highlightId)` 打开阅读器 Tab 并跳到标注位置；评论区内 Markdown `<a>` 链接用 `onClickCapture` 判断后 `stopPropagation`，避免误触发跳转
...
- **资料列表**：搜索栏和排序栏固定在顶部（`flex-shrink: 0`），列表独立滚动（`.listScroll` 容器 `overflow-y: auto`）。每项显示最多 3 个标签色点 + 高亮数量徽章。搜索时匹配类型标签（正文/标注/评论）独立行显示，snippet 显示 2 行截断。**排序**：`SortBy` 三档 `created_at`（创建/收藏时间）/ `content_time`（资料时间）/ `annotated_at`（标注时间），`content_time` 与 `annotated_at` 均走 `ORDER BY COALESCE(...)` 回退到 `created_at`（未回补资料时间的资料按收藏时间排）；按资料时间排序时列表项日期列显示 `content_time`（无则 `created_at`）。`SortBy` 是 `#[serde(rename_all="snake_case")]` 枚举，三处 exhaustive match（`cmd_search_resources` 转 str、search.rs/server.rs 字符串→枚举、`list_*` SQL 分支）新增档位时都要补
- **资料时间（content_time）**：迁移 010 给 `resources` 加 nullable `content_time TEXT`（存 `YYYY-MM-DD`），语义为"内容本身的时效性"（如文章发布日），区别于 `created_at`（加入拾贝时间）/ `captured_at`（抓取时间）。抓取时默认 NULL，经编辑对话框回补。作为 `Resource` struct 字段自动随 sync_log payload + 全量快照 export/import + `upsert_resource`（LWW，ON CONFLICT 更新 content_time）同步；新增列须同步改全部 `Resource` 构造点（resources.rs 的 get/list×2/find_by_url、search.rs、tags.rs）。`update_resource(conn,id,title,description,url,content_time,ctx)` 扩展为可改 url + content_time（HTTP `handle_update_resource` 同步支持，缺省字段保留原值）
- **资料 Tab 右键菜单**：reader（资料）Tab 右键复用 `ResourceContextMenu`（在 `App.tsx` 顶层渲染，`TabBar` 经 `onTabContextMenu` 回调上抛、仅对 `readerTabs` 命中的 Tab `preventDefault` 弹菜单）。提供复制链接 / 添加标签 / 移动到目录 / 关联问题 / 编辑（打开 `ResourceEditDialog`，保存后 refetch 刷新 Tab 标题），**不含删除**（`ResourceContextMenu` 新增 `showDelete?: boolean` 默认 true，Tab 菜单传 `false` + 省略 `onDelete` 隐藏破坏性操作）。移动后即时更新 `readerTabs` 内 `resource.folder_id` 保持后续菜单 currentFolderId 正确
...
- **音频支持**：`resource_type = "audio"`，复刻 PDF 接入模式。本地导入 `cmd_import_audio`（保留原扩展名落盘 `snapshot.{ext}`，校验白名单 `AUDIO_IMPORT_EXTENSIONS`），无插件抓取、无应用内录音。`AudioReader` 组件（HTML5 `<audio>` + 播放/倍速/±15s + 时间轴 marker），经 `shibei://resource/{id}` 协议返回原始字节 + **HTTP Range 206**（`serve_file_with_range`/`find_audio_snapshot`/`audio_mime`/`parse_byte_range` in `lib.rs`）流式 seek。标注 anchor `{ type: "audio", start, end, charIndex?, length?, textQuote? }`：纯时间段 marker 只有 start/end；转写文本高亮带 char/quote。**同步扩展名派生**：sync `snapshot_filename`/`snapshot_s3_key` 改收 `ext`，从 `file_path` 派生（`ext_for`/`snapshot_ext`，仅 audio 查 DB，避开 size-1 池双借用），`download_snapshot` audio 分支不从字节提取 plain_text。**转写 = 外部 agent 经 MCP 委托**（拾贝不做 ASR）：`set_transcript(id, text?, language?, segments[])` HTTP 写 `storage/{id}/transcript.json`（`shibei-storage` 的 `transcript` feature）+ `set_plain_text`（重建 FTS，音频可全文搜索）；`get_resource` 对 audio 返 `abs_path` 供 agent 读文件。`cmd_get_transcript` 给前端；AudioReader 转写视图（点句跳播 + 卡拉OK 跟随 + 选转写文本建高亮 + 段落按高亮着色，经 `data:resource-changed` 实时刷新）。**transcript.json 同步**（B3）：`set_transcript` 打 `transcript_upload_pending:{id}` 标记 → 同步上传 `transcripts/{id}.json.gz`（可覆盖，记录 S3 etag）；下载相每个活跃 audio 资源 `head` 比对 etag（异则下载 + 重派生 plain_text），etag 变更检测同时覆盖首传与重转写传播，**不动 resource LWW、无 migration**。详见 `docs/superpowers/specs/2026-05-31-audio-support-design.md`
...
- **文件导入入口**：通过两个右键菜单触发（已移除第二栏顶部的 "PDF+" 按钮）：①Sidebar 文件夹行右键 → "导入文件"；②ResourceList 空白处右键（选中真实 folder 时）→ "导入文件"。两处共享 `src/lib/importFile.ts` 的 `importFileToFolder(folderId)`（原 `importPdf.ts`），文件对话框 filter 含 PDF + 音频（`pdf` + `AUDIO_EXTENSIONS`），按扩展名分派 `importPdf`/`importAudio`，错误通过 `translateError()` 翻译。i18n key 为 `reader.importFile`（文案"导入文件"）
...
- **鸿蒙会话持久化 / Deep Link / 同步刷新（Phase 5 收尾，2026-04-24）**：`app/SessionState.ets` 使用 `@ohos.data.preferences` store=`shibei_prefs` key=`shibei-session-state`，只存移动端导航上下文（`inReader` / `readerResourceId` / `library.selectedFolderId|selectedTagIds|selectedResourceId`；`listScrollTop` 字段保留但当前未写入）；Reader scroll 和 PDF zoom 继续分别由 `ReaderScrollState.ets` / `ReaderZoomState.ets` 的 per-resource map 管理。打开 Reader 前必须 `await SessionState.save({inReader:true,readerResourceId})` 再 `router.pushUrl`，避免快速 kill 时尚未 flush；**不得在 `Reader.aboutToDisappear()` 清 `inReader`**，因为任务管理器杀 app 也会触发该生命周期；只在显式返回（系统 back / 顶部 `←`）通过 `leaveReader()` 清。`module.json5` 注册 `shibei://open/resource/{id}?highlight={hlId}` 与 `shibei://open/folder/{id}`，`EntryAbility.onCreate` 暂存 `Want.uri` 到 `KEY_PENDING_DEEP_LINK`，`Library.aboutToAppear` 优先消费 pending deep link（资源则 push Reader，文件夹则切换列表）再恢复旧 Reader 会话；`EntryAbility.onNewWant` 在已解锁态直接 push Reader，文件夹则暂存后回到 Library 消费，锁屏态暂存等待 Library 消费；`Reader` 读取 `highlightId` 后 ready 时 flash；`ResourceList`/`FolderDrawer` 长按菜单分别提供复制 `shibei://open/resource/{id}` / `shibei://open/folder/{id}`。`ShibeiService.syncMetadata()` 远端 apply/download 后触发 `onDataChanged`，`ResourceList` 与 `FolderDrawer` 都必须订阅以保持自动同步后的列表新鲜度
...
- **鸿蒙 i18n + 深色主题（Phase 5.1）**：ArkUI 走原生 `resourceManager.getStringSync(res, ...args)` 做 i18n，不引第三方库。`services/I18n.ets` 单例封装（try/catch + null-context guard + `currentLocale()` 返 `'zh'|'en'`），`EntryAbility.onWindowStageCreate` 第一行调 `I18n.init(ctx)`。资源布局：`resources/{zh_CN,en_US,base}/element/string.json` 三份（base 作 fallback，镜像 zh 内容 + 4 条 entry_label/perm_reason 鸿蒙系统 key）；key 命名沿桌面 namespace 用 `_` 分割（`common_*` / `settings_*` / `onboard_*` / `reader_*` / `annotation_*` / `lock_*` / `search_*` / `sidebar_*` / `resource_list_*` / `huks_*`），占位符用鸿蒙原生 `%1$s` / `%1$d`。**新增 UI 文案必须**：①在三份 JSON 同步加同名 key；②用 `I18n.t($r('app.string.key'), ...args)` 而非硬编码 CJK。**field initializer 陷阱**：组件 `@State foo: string = I18n.t(...)` 会在 EntryAbility 之前求值拿空串——用 `aboutToAppear` 里赋值。**`$r()` 必须收字面量 key 陷阱**：`I18n.t($r(cond ? 'app.string.a' : 'app.string.b'))` 编译通过但运行期解析成空串（`$r` 是编译期宏，要字面量）；三元要写在 `$r` 外面——`cond ? I18n.t($r('app.string.a')) : I18n.t($r('app.string.b'))`（对齐 QuestionEdit/QuestionNoteEdit 标题写法）。色板 `resources/base/element/color.json`（~20 token：`bg_*` / `text_*` / `accent_*` / `border_divider` / `shadow_default` / `surface_tap_invisible`）+ `resources/dark/element/color.json` 同名 override，鸿蒙系统按 ColorMode 自动选；**HL_COLORS 不 tokenize**（`#ffeb3b/#81c784/#64b5f6/#f48fb1` 是跨端标注数据，不是样式）；`promptAction.showDialog` 的 button.color 字段接 ResourceColor（`$r('app.color.*')`）。**dialog 按钮用 token**：cancel→text_secondary、confirm→accent_primary/accent_primary_alt、destructive→accent_danger。WebView 深色：`Reader.ets` 的 `Web` 组件 `.darkMode(WebDarkMode.Auto)`（HTML 快照 + PDF shell 都加）+ **不设** `forceDarkAccess(true)`（避免 Chromium 强制反色破坏快照原貌，对齐桌面行为）；pdf-shell 内部 `style.css` 用 `@media (prefers-color-scheme: dark)` 只改 chrome（body/status/.page shadow），**canvas 内容保持白底**（PDF 内容永远是白底，和 Acrobat/桌面一致）；pdf-shell 的 i18n 通过 Web src URL param `&loadingText=<encodeURIComponent(I18n.t(common_loading))>` 注入，main.js `params.get('loadingText')` 读取 setStatus。详见 `docs/superpowers/plans/2026-04-23-harmony-i18n-theme.md` + `docs/superpowers/plans/2026-04-23-harmony-i18n-keys.md`
- **鸿蒙 Questions（Phase H，2026-05-28；Detail 资料聚合 2026-06-02；研究笔记 2026-06-02）**：19 个 NAPI 命令（list/get/create/update/archive/unarchive/delete/list_links/**list_resolved_links**/list_for_target/link/update_link_reason/unlink/search + **list/create/update/delete_question_note** + 2 个 lookup `get_highlight`/`get_comment`），全部通过 `shibei-napi-codegen` 生成；ArkTS 入口 `services/QuestionService.ets` 是单例 façade，持三层缓存（active/archived list snapshot、**per-question resolved links**、per-target reverse-lookup）+ 各自订阅器；构造时自挂 `ShibeiService.onDataChanged`，远端同步完成自动 invalidate UI。UI：`pages/Questions.ets`（active/archived 折叠组 + + 按钮 + 长按菜单：编辑/复制链接/归档/删除）；`pages/QuestionDetail.ets`（状态徽章 + Markdown 描述 + **按资料聚合的链接卡**：每份资料一张卡只出现一次，整篇关联理由 + 嵌套的高亮/笔记证据，高亮带原色左竖条、笔记走 `MarkdownText` 渲染 + >140 字折叠、行 `.stateStyles` 按压反馈 + 点击跳转 Reader + ⋯ 菜单）；`pages/QuestionEdit.ets`（创建/编辑表单）；`components/QuestionLinkSheet.ets` 是底部 sheet，**点击 toggle 后自动关闭**（对齐桌面 LinkToQuestionSubMenu），行用 `@Styles` + `.stateStyles({ pressed, normal })` 加按压反馈。`components/ResourceItem.ets` 卡片底部显示反查 chip 行（≤3 个 + "+N" 溢出）。**链接解析走 `list_resolved_question_links`**（后端一次返回 link + 父 Resource + snippet + anchor，复用桌面共享的 `shibei-db::list_resolved_links_for_question`），不再 `listResources('__all__')` 逐资源扫 annotations；分组/排序逻辑（`rebuild()`/`anchorSortKey`，directly-linked 资料优先 + 高亮按文档位置）端侧与桌面 `groupResolvedLinks` 对齐。**ArkUI @Builder 限制**：禁止在 @Builder 体内写 `const` / `if-return`——QuestionDetail 把分组/排序搬进普通方法（`rebuild()` 写 `@State groups`），@Builder 只调辅助方法（`groupBadge`/`noteNeedsFold`/`isNoteExpanded` 等）。Deep link `shibei://open/question/{id}` 在 `EntryAbility.onNewWant`（已运行）和 `Library.consumePendingDeepLink`（冷启动/锁屏后）双路径处理。**评论无长按菜单**——只 note 卡（`CommentCard` builder，实际仅渲染 highlight_id IS NULL 的 comment）有 `NoteContextMenu`。**研究笔记（Phase 2，2026-06-02）**：`QuestionService` 加第四层缓存 `notesCache`（per-question）+ `subscribeNotes`/`loadNotes`/`refreshNotes` + `create/update/deleteQuestionNote`（mutation 后 refreshNotes，onSyncCompleted 循环刷新）；`QuestionDetail` 在描述与关联之间渲染「研究笔记」区（`@Builder NoteCard`：时间戳 + `MarkdownText` 渲染 + 点卡片跳 `pages/QuestionNoteEdit` 编辑 + ⋯ 菜单编辑/删除，+ 按钮新建）；`pages/QuestionNoteEdit.ets` 单 TextArea 编辑器（create 收 `questionId`，edit 额外收 `noteId`+`content` 经 router 传入，免单条 fetch 命令）。移动端笔记区**不做整区折叠**（桌面的折叠是为窄三栏，移动端全屏滚动无需）。**搜索匹配提示**：`searchQuestions` 返回 `QuestionSearchResult`（flatten Question + `matchFields` + `snippet`，JSON 向后兼容老 `Question[]` 解析）；`QuestionListView` 搜索命中描述/笔记时，行内显示「笔记匹配/描述匹配」标签 + snippet（与桌面对齐；移动端 snippet 不做查询词高亮）。注意 `search_questions` NAPI 签名（String→String）没变,但返回 JSON 加了字段,**改 search.rs 后须重编 .so** 才能让鸿蒙拿到 snippet
...
- **鸿蒙音频（Phase C）**：`Reader.ets` 按 `resource_type === 'audio'` 分支到 `@Builder AudioContent()`。**播放**：`@kit.MediaKit` 的 `media.AVPlayer` 经 `fdSrc`（`fs.openSync(path).fd`）直读沙箱文件——路径由 NAPI `ensure_audio_downloaded(id)`（async，下载缺失音频 + 返回**绝对路径**）给出，**绝不 base64 大音频过 NAPI**（区别于 PDF）。`aboutToDisappear` 必 `release()` + `fs.closeSync(fd)`。倍速受 `media.PlaybackSpeed` 枚举限制（0.75/1/1.25/1.75/2，**无 1.5**）。**转写**：NAPI `get_transcript(id)`（sync）返回 transcript.json **原始字符串**，ArkTS `JSON.parse`（transcript 由 sync 的 `download_remote_transcripts` 下载，没拿到则转写区不显示）。`List` + `transcriptScroller`：点句 `seekTo`，卡拉OK 跟随用 `scrollToIndex(idx, true, ScrollAlign.CENTER)`；**`seekTo` 显式滚动**（不只播放时跟随），`updateActiveSeg(follow)` 的 follow **不再 gate `isPlaying`**（否则点高亮只跳音频不滚字幕——桌面同 bug 同修）。**关键约束（闪退）**：音频阅读器**没有 Web 组件**绑 `webController`，所以 `flashHighlight`/`deleteHighlight`/`clearSelection`/`paintHighlights` 里的 `webController.runJavaScript` 对 audio **必须 `isAudio()` guard 掉**——它**同步抛异常**（`.catch` 拦不住）直接闪退。点高亮卡 → `flashHighlight` 对 audio 改为 seek 到 `anchor.start`。**anchor 解析**：`Highlight.anchor` 是 `Object`，用 `as AudioAnchorLite`（接口）读 `type/start/end`（同 `router.getParams() as ...` 套路）。段落着色 `highlightTintForSeg`（半透明 `#66RRGGBB`，半开区间重叠不渗邻段）。**创建标注（无自由选文本）**：长按转写句弹 `SegmentMenu`——4 色 = 标这一句 / 「多选标记」= 进区间模式；多选下点起点 + 点终点，连续区间打勾着色（**自绘 18×18 固定方框，不用 `☑/☐` 字形**——两字形跨字体大小不一），底部 `MultiSelectBar` 选色建**一条跨多句**高亮，多选模式下点击=选行不跳播。anchor `{type:'audio', start:首句.start, end:末句.end, textQuote:{exact}}`，与桌面同构、LWW 互通。**侧栏**：`AudioContent` 包在 `Stack({alignContent:Bottom})`，`panelOpen` 时叠透明 `surface_tap_invisible` scrim 点击收回抽屉（同 HTML/PDF）。**NAPI**：`ensure_audio_downloaded`/`get_transcript` 两个新命令经 codegen 生成。**MVP 限制**：无本地导入（移动端不导入音频）、无应用内录音、无转写文本子句级精度（按段落为单位）
