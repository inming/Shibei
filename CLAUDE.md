...
- **搜索**：FTS5 trigram 虚拟表 `search_index`，CRUD 操作后自动维护索引（best-effort），前端 ResourceList 顶部搜索框 debounce 300ms 触发，标题/URL/标注匹配高亮
...
- **全文搜索**：FTS5 虚拟表 `search_index`（`tokenize='trigram'`），索引 title/url/description/highlights_text/comments_text。>= 3 字符走 FTS5 MATCH，2 字符回退 LIKE。所有 CRUD（resource/highlight/comment）操作后调用 `search::rebuild_search_index` 或 `delete_search_index`（best-effort `let _ =`）。同步 apply 后按受影响 resource_id 增量重建。首次启动通过 `sync_state` 标志位 `config:fts_initialized` 触发全量索引构建。FTS5 查询必须用 `escape_fts_query` 转义用户输入（双引号包裹）
