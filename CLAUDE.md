...
src-tauri/          # Rust 后端（Tauri core）
  src/
    commands/       # Tauri command handlers（26 个命令）
    db/             # 数据库操作（migration、folders/resources/tags/highlights/comments CRUD，含软删除+HLC）
    server/         # 本地 HTTP server（axum，插件通信）
    storage/        # 文件系统存储逻辑
    sync/           # S3 云同步（HLC 时钟、sync_log、sync_state、SyncBackend trait、SyncEngine、凭据、全量导出）
...
- 优先选择轻量、维护活跃的库
- 当前 Rust 依赖：tauri, rusqlite(bundled), r2d2/r2d2_sqlite, axum, tokio, tower-http, thiserror, uuid, chrono, serde/serde_json, dirs, base64, rust-s3, async-trait
