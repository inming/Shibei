...
- **设置页**：独立 Tab（`__settings__`），侧栏导航分区（外观 / 同步 / 加密 / 安全 / 数据 / AI），`SettingsView` + `Settings/AppearancePage` + `Settings/SyncPage` + `Settings/EncryptionPage` + `Settings/LockScreenPage` + `Settings/DataPage` + `Settings/AIPage`
